...
$$\text{MMD}(P_\star, P) = \| \mu_k(P_\star) - \mu_k(P)\|_{\mathcal{H}(k)}$$ \add{Note: Changed MMD to \text{MMD} so it is not formatted as the variables M times M times D.}
 - proper metrix if $k$ is the characteristic kernel
 - Statistical divergence by "proper scoring rule [Dawid, 1986]" \add{Note: Closed the quotation mark here.}

\add{Note: Removed HTML <br> tag which will break LaTeX compilation or render literally. If you want vertical space, use \vspace{1em} instead.}
...
which means that 
$$MMD^2(P_\star, P) = \left\| \int \{ \mu_k(P_\star) - \mu_k(P_\theta) \} \add{dQ(\theta)}\right\|^2_{\mathcal{H}(k)} = \int\int\kappa_{P_{\star}}(\theta, \vartheta) dQ(\theta)dQ(\vartheta)$$ \add{Note: Added missing dQ(\theta) inside the norm. Changed \mathbb{v} and \mathscr{v} to \vartheta to match your later equations. \mathbb{v} does not exist in standard amsfonts and will trigger a fatal compilation error.}
...
- data vs model 1
- data vs model 2
- model 1 vs model 2 where \add{Note: Consider wrapping these bullet points in an itemize environment, as raw dashes in text mode will not render as a proper list.}
...
---- \add{Note: These four dashes will render exactly as written. If you want a horizontal line, replace them with \noindent\hrulefill}
...
\add{Note: Removed $$ from around the align block below. The align environment natively initiates math mode, so wrapping it in $$ causes a fatal compilation error.}
\begin{align}
...
\newpage
\section{Other notes for later chapters:}
